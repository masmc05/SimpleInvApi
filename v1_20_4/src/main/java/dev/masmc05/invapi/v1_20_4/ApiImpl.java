...
public class ApiImpl implements Api {
    ...
    @Override
    public @NotNull Inventory createFlexibleInventory(int size) {
        return new CraftInventory(new SimpleContainer(size));
    }
    ...
    public @NotNull Inventory createSimpleCopy(@NotNull Inventory inventory) {
        if (inventory instanceof CraftInventory craftInventory) {
            ...
            var inv = this.createFlexibleInventory(inventory.getSize());
            for (int i = 0; i < inventory.getSize(); i++) {
